...
Monitor.prototype.getCollection = function(group) {
  ...
  if (!this._collections[name]) {
    this._collections[name] = {
      collection: new measured.Collection(name),
      lastUpdated: Date.now(),
    };
  }
  this._collections[name].lastUpdated = Date.now();
...
Monitor.prototype.forEachCollection = function(fn) {
  ...
  Object.keys(self._collections).forEach(function(name) {
    if (Date.now() - self._collections[name].lastUpdated > self._ttl) {
      self._collections[name].collection.end();
      delete self._collections[name];
      return;
    }

    fn(self._collections[name].collection);
  });
};
...
Monitor.factory = function(url, interval, collectionFilter, ttl) {
